...
const Marks = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          setInternal(response.data.marks.internal);
          setExternal(response.data.marks.external);
        } else {
          toast.error(response.data.message);
        }
      })
  ...
  const downloadMarks = () => {
    const data = JSON.stringify({ internal, external });
  ...
  return (
    <div className="w-full mx-auto mt-10 flex justify-center items-start flex-col mb-10">
      ...
      <button className="bg-blue-500 px-6 py-3 mt-8 mx-auto rounded text-white" onClick={downloadMarks}>
        Download Marks
      </button>
    </div>
  );
