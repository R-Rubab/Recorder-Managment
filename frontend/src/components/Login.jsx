...
const Login = () => {
  ...
  const onSubmit = (data) => {
    if (data.login !== "1" && data.password !== "1234") {
      const headers = {
    ...
    <div className="bg-white h-[100vh] w-full flex justify-between items-center">
      ...
      <div className="w-[40%] flex justify-center items-start flex-col pl-8">
        ...
        >
          ...
          <button onClick={navigateDirectly} className="bg-blue-500 mt-5 text-white px-6 py-2 text-xl rounded-md hover:bg-blue-700 ease-linear duration-300 hover:ease-linear hover:duration-300 hover:transition-all transition-all flex justify-center items-center">
            Login
            <span className="ml-2">
              <FiLogIn />
            </span>
